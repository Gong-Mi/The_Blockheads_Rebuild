#!/usr/bin/env python3
"""Generate the batch-2i pairing table mechanically from annotated listings
+ freeblock simulator (same pipeline as gen_b2h_table, parameterised).

Pool starts are the first literal-cell address of each body (verified by the
simulator needing no cell past code end)."""
...
BODIES = {
 'Sign':           ('disasm_sign_getsavedict.txt',          0x005FAEB4, 0x005FB1CC),
 'ElevatorMotor':  ('disasm_elevatormotor_getsavedict.txt',0x00700B5C, 0x00700E8C),
 'ElevatorShaft':  ('disasm_elevatorshaft_getsavedict.txt',0x00CAD998, 0x00CADCCC),
 'Painting':       ('disasm_painting_getsavedict.txt',     0x00AA8FC8, 0x00AA9340),
}
    ...
    for cls, (listing, imp, pool) in BODIES.items():
        ...
        for r in sites:
            ...
            if sel and sel.startswith('numberWith'):
                ...
                ivars = [f'OBJC_IVAR_$_{cls}.{base}',
                         f'OBJC_IVAR_$_{cls}.{key}',
                         f'OBJC_IVAR_$_DynamicObject.{key}',
                         f'OBJC_IVAR_$_InteractionObject.{key}']
                iv = None
                for m in re.finditer(
                    ...
                    if sym in ivars and (iv is None or a > iv[0]):
