// Flat C ABI bridge for the b4o VinePlant loader differential.
...
namespace { constexpr std::uint32_t kMaxTiles = 64; }

typedef struct VineBridgeInput {
    ...
    std::uint32_t tile_byte7[kMaxTiles];
    std::uint32_t tile_byte11[kMaxTiles];
    std::uint32_t tile_half_a[kMaxTiles];
    std::uint32_t tile_half_b[kMaxTiles];
    std::uint32_t tile_half_c[kMaxTiles];
} VineBridgeInput;

extern "C" {

std::uint32_t recovered_vine_plant_init_run(const VineBridgeInput* cfg,
    ...
    for (std::uint32_t i = 0; i < cfg->tile_count; ++i) {
        VineTileAnswer answer;
        answer.present = cfg->tile_present[i] != 0;
        answer.byte0 = static_cast<std::uint8_t>(cfg->tile_byte0[i] & 0xff);
        answer.byte7 = static_cast<std::uint8_t>(cfg->tile_byte7[i] & 0xff);
        answer.byte11 = static_cast<std::uint8_t>(cfg->tile_byte11[i] & 0xff);
        answer.half_a = static_cast<std::uint16_t>(cfg->tile_half_a[i] & 0xffff);
        answer.half_b = static_cast<std::uint16_t>(cfg->tile_half_b[i] & 0xffff);
        answer.half_c = static_cast<std::uint16_t>(cfg->tile_half_c[i] & 0xffff);
        in.tiles.push_back(answer);
    }

    const auto res = blockheads::recovered::vine_plant_init_with_world(in);
    for (std::size_t i = 0; i < blockheads::recovered::kVineImageSize; ++i) {
        image_out[i] = res.image[i];
    }
    *return_out = res.return_value;
    *tiles_out = static_cast<std::uint32_t>(res.tiles_consumed);
    const std::uint32_t n = res.calls.size() > blockheads::recovered::kVineMaxTrace
                                ? blockheads::recovered::kVineMaxTrace
