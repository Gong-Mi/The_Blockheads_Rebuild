...
void main()
{
    ...
    highp float specular = 0.3 * (pow(diffuse, 12.0 * 0.3) * 0.15 + diffuse * 0.2) * 0.5;
    
    
    gl_FragColor = (hairColor + vec4(vec3(specular), 0.0)) * vec4(light * vec3(diffuse,diffuse,diffuse * 0.9 + 0.1), 1.0);
}
