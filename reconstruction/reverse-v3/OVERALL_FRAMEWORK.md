...
```text
既有 APK gameplay（非原版等价实现）
  → 挖掘等待实际移除 → 生成掉落 → 满包保留/可入包再删除
  → 16位物品放置 → 真实CPU mesh重建/meshReady
  → 已完成状态保存world.bin → 真loader重载全部Tile/库存 [本地行为测试通过]
制作并行线：扣料一次 → 计时 → 满包/部分交付留存 → 入包数量回报 → JNI热栏dirty接线
原版恢复线：pickup容量分支原指令核对，但完整原版状态机/subItems/所有权仍待恢复
验收分层：O0/O2各19项CTest通过 → exact-head Android CI/APK另验 → 真机前台未运行
未闭合：中途craft/drop持久化、原版动作/寻路/配方、完整视觉链和游戏真机验收
```

本批实改 app/，不是仅新增恢复库；但不增加原版已恢复完整方法数。
证据、RED→GREEN、生产接线和明确边界见 `GAMEPLAY_LOOP_REPAIR.md`。

## 服务器证据与客户端离线组装（新增独立施工线）

```text
原版Linux服务器1.7.1（固定SHA，带DWARF）
  ├→ 创建世界→落盘→重载：已实跑（QEMU CPU，非Android客户端）
  ├→ 停服快照：3个LMDB环境/53条记录/11个实体文件
  │   ├→ 40条gzip区块→真实客户端C++解码器→逐字节比对：O0/O2通过
  │   ├→ 13条plist→16个动态对象及世界元数据：无损保留，尚未构造游戏对象
  │   └→ blob归档→恢复实体文件→原服重新加载：已实跑
  ├→ Tile/PhysicalBlock及全部53组具名枚举：1120定义无冲突合并，1817枚举项
  └→ ENet禁用探针压缩后CONNECT成功，收到243字节worldID消息；游戏握手待恢复

统一本地验收：恢复方法6项 + 数值切片5项 + 存档解码1项
  → CMake Debug/Release各12/12，Release测试保留断言
下一接入边界：原始区块/动态对象→真实World状态→renderer；不写空World.update糊接
```

工具及复现边界见 `SERVER_CLIENT_ASSEMBLY.md`。原版二进制、生成存档及实验日志不提交进git。
